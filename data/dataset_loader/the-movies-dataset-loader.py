...
def ingest_data():
    dataframe = pd.read_csv(path.join('.', 'data', 'datasets', 'the-movies-dataset', 'movies_metadata.csv'))

    dataframe["genres"] = dataframe["genres"].apply(json_formatter)
    dataframe["production_companies"] = dataframe["production_companies"].apply(json_formatter)
    dataframe["production_countries"] = dataframe["production_countries"].apply(json_formatter)
    dataframe["spoken_languages"] = dataframe["spoken_languages"].apply(json_formatter)

    formatted_data = dataframe.to_json(orient='records')

    bulk_data = []
    for item in json.loads(formatted_data):
        tmp_item = {**item, "completion": [item['title']], "search-as-you-type": item['title']}
        bulk_data.append({"index": {"_index": "movie-metadata", "_id": item['id']}})
